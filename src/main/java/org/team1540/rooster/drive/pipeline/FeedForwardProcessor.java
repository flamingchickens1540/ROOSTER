...
public class FeedForwardProcessor implements Processor<TankDriveData, TankDriveData> {
   ...
   *
   * @param velocityFeedFwd The velocity constant feed-forward \(k_v\), in output units per speed
   * unit.
   * @param throtBump The velocity intercept \(V_{intercept}\), in output units.
   * @param accelFeedFwd The acceleration constant feed-forward \(k_a\), in output units per
   * acceleration unit.
   */
   ...
   * <li>The product of the acceleration (if present) and the acceleration feed-forward is
   * added.</li>
  ...
  public TankDriveData apply(@NotNull TankDriveData command) {
    return command.plusAdditionalFeedForwards(
        getThrottle(command.left.velocity.orElse(0), command.left.acceleration.orElse(0)),
        getThrottle(command.right.velocity.orElse(0), command.right.acceleration.orElse(0)));
  }
}
